...
(use-package autoload
  :straight (:type built-in)
  :init
  (add-to-list 'load-path thomas-generate-autoload-dir)
  (defun thomas-generate-autoload-define (loaddef &rest DIRS)
    "Collect all autload function in autoload file."
    (interactive)
    (let ((generated-autoload-file loaddef))
      (when (not (file-exists-p loaddef))
        (with-current-buffer (find-file-noselect generated-autoload-file)
          (insert ";; generated by function: `thomas-generate-autoload-define'.")
          (save-buffer)))
      (apply 'update-directory-autoloads DIRS)))

  (defun thomas-reload-core-autoloads ()
    "Reload autoload function from `core/autoload' and save in `thomas-autoload-file'."
    (interactive)
    ...
    (thomas-generate-autoload-define thomas-autoload-file thomas-core-autoload-dir)
    (load thomas-autoload-file nil 'nomessage)
    (message "Reload core autoload file: %s done." thomas-autoload-file))

  (defun thomas-load-core-autoload ()
    "Load `thomad-autoload-file' autoload file"
    (when (not (file-exists-p thomas-autoload-file))
      (thomas-generate-autoload-define thomas-autoload-file thomas-core-autoload-dir)
      (byte-compile-file thomas-autoload-file)
      (message "generate autoload file: %s done." thomas-autoload-file))
    (load thomas-autoload-file nil 'nomessage))

  ;; 添加检查 autoload 文件
  (add-hook 'after-init-hook  #'thomas-load-core-autoload))
